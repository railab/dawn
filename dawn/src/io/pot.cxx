      ...
      switch (item->cfgid.s.id)
        {
          case IO_POT_CFG_WIPER:
            if (item->cfgid.s.size != 1)
              {
                DAWNERR("invalid POT wiper config size %d\n", item->cfgid.s.size);
                return -EINVAL;
              }
...
int CIOPot::configure()
{
  ...
  // Wiper index must exist on the device

  ret = pot_get_nwipers(fd, &nwipers);
  if (ret < 0)
    {
      DAWNERR("pot_get_nwipers failed %d\n", ret);
      deinit();
      return ret;
    }
  ...
  if (wiper >= nwipers)
    {
      DAWNERR("POT wiper %u out of range (%u wipers)\n", wiper, nwipers);
      deinit();
      return -EINVAL;
    }
