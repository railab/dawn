  ...
  for (i = 0; i < inputs.size(); i++)
    {
      ...
      // Reject a mask that would be shifted out of the output word entirely
      // or in part - a silently truncated field is the bug this program
      // exists to avoid.
  ...
  for (i = 0; i < inputs.size(); i++)
    {
      ...
      // Fields are merged as plain words, so the element must be one

      if (!prog::isBitwiseDtype(io->getDtype()) || io->getDtypeSize() > sizeof(uint32_t))
        {
          DAWNERR(
...
int CProgBitMerge::ioNotifierCb(void *priv, io_ddata_t *data)
{
  ...
  // Slow inputs are re-read by mergeConstant(), so nothing is cached here.
  // A change does not republish the stream; the next batch picks it up.
