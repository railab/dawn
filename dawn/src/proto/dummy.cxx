  ...
  while (offset < getDesc().getSize())
    {
      ...
      if (item->cfgid.s.cls != CProtoCommon::PROTO_CLASS_DUMMY)
        {
          DAWNERR("dummy proto: unsupported cfg 0x%08" PRIx32 "\n", item->cfgid.v);
          return -EINVAL;
        }

      switch (item->cfgid.s.id)
        {
          case PROTO_DUMMY_CFG_IOBIND:
            {
              size_t i;
