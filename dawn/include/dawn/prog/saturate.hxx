 ...
 * output can hold is what makes the narrowing store safe. Integer, bool,
 * float, double and b16 types are supported; bound words follow the input
 * type (b16 clamps raw and does not convert to other types).
 */
...
private:
  ...
  double lo;                 ///< Decoded lower bound.
  double hi;                 ///< Decoded upper bound.
  std::mutex boundsLock;     ///< Guards lo/hi against a concurrent config write.
