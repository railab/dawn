// dawn/include/dawn/io/encoder_index.hxx
//
...
namespace dawn
{
/** @brief Quadrature encoder I/O (position + index information). */

class CIOEncoderIndex : public CIOCommon
{
public:
  enum
  {
    IO_ENCODER_INDEX_CFG_FIRST = 0,
    IO_ENCODER_INDEX_CFG_POSMAX = 1,
    IO_ENCODER_INDEX_CFG_LAST = 31
  };

  explicit CIOEncoderIndex(CDescObject &desc)
    : CIOCommon(desc)
    , fd(-1)
    , posmax(0)
    , hasPosmax(false)
  ...
  using ObjectIdHelper =
    CIOCommon::IOObjectIdHelper<IO_CLASS_ENCODER_INDEX, SObjectId::DTYPE_INT32>;
  ...
  constexpr static SObjectCfg::ObjectCfgId cfgId(bool rw, uint8_t dtype, uint8_t size, uint8_t id)
  {
    return SObjectCfg::objectCfg(
      SObjectId::OBJTYPE_IO, CIOCommon::IO_CLASS_ENCODER_INDEX, dtype, rw, size, id);
  }

  constexpr static SObjectCfg::ObjectCfgId cfgIdPosmax()
  {
    return CIOEncoderIndex::cfgId(false, SObjectId::DTYPE_UINT32, 1, IO_ENCODER_INDEX_CFG_POSMAX);
  }
