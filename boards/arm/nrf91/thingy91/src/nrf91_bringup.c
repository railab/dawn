...
int nrf91_bringup(void)
{
  ...
  /* Mount the tmpfs file system at the configured temporary directory so
   * applications have a writable scratch area (there is no NSH/rcS to do it).
   */
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR,
             "ERROR: Failed to mount the TMP filesystem: %d\n",  ret);
    }
  ...
  /* Bring up the ADP5360 3.3 V buck-boost rail before the 3.3 V peripherals
   * (BH1749 colour sensor, RGB LEDs).  It is disabled at power-on reset.
   */
