/****************************************************************************
 * boards/arm/nrf52/ppk2/src/nrf52_composite.c
 *
...
static void *board_composite0_connect(void)
{
  ...
  /* Configure the CDC/ACM device: /dev/ttyACM0 carries the nxscope data,
   * the console runs on RTT (see CONFIG_SERIAL_RTT_CONSOLE).
   */
  ...
  for (i = 0; i < 1; i++)
    {
      ...
      /* Interfaces: offset to interface IDs and the minor number */

      dev[dev_idx].devinfo.ifnobase = ifnobase;
      dev[dev_idx].minor = i;

      /* Strings: offset to string numbers */
