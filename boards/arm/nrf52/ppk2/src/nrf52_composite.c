...
void *board_composite0_connect(void)
{
   ...
   *   /dev/ttyACM0 - nxscope data
   *
   * Only a single serial port fits alongside the other USB endpoints; the
   * NuttShell console runs on RTT (see CONFIG_SERIAL_RTT_CONSOLE).
   */
  ...
  for (i = 0; i < 1; i++)
    {
      ...
      dev[dev_idx].minor = i;                           /* The minor interface number */
